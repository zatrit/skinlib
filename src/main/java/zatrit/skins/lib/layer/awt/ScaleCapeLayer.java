...
/**
 * Calculates the smallest power of the number 2 that is greater than the height of the image and
 * creates a new image with a width equal to the two new heights.
 */
@Setter
public class ScaleCapeLayer extends ImageLayer {
  ...
  public BufferedImage apply(@NotNull BufferedImage image) {
    ...
    if (this.elytraTexture != null && image.getWidth() != width) {
      graphics.drawImage(this.elytraTexture, 0, 0, result.getWidth(), result.getHeight(), null);
    }
