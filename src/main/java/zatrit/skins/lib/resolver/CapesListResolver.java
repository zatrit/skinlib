...
public abstract class CapesListResolver implements Resolver {
  ...
  public @NotNull PlayerTextures resolve(@NotNull Profile profile) throws Exception {
    ...
    if (capeName != null) {
      textures.put(TextureType.CAPE, new URLTexture(this.getUrl(capeName), null));
    }

    return new CachedPlayerTextures<>(textures, this.config.getLayers(), this.config.getCache());
  }
}
