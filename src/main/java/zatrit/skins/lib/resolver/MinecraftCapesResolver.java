...
/**
 * Resolver for <a href="https://minecraftcapes.net/">Minecraft Capes</a> based on the behavior of
 * the Minecraft Capes mod. Connects to the API at {@code https://api.minecraftcapes.net/}
 *
...
public final class MinecraftCapesResolver implements Resolver {
  ...
  public @NotNull PlayerTextures resolve(@NotNull Profile profile) throws IOException {
    ...
    for (val entry : response.getTextures().entrySet()) {
      ...
      val texture = new BytesTexture(textureData, decoder.decode(textureData), metadata);
