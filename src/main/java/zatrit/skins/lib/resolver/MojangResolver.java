...
/** <a href="https://wiki.vg/Mojang_API">Mojang API</a> implementation for OpenMCSkins. */
@AllArgsConstructor
public final class MojangResolver implements Resolver {
  private static final String MOJANG_SKIN_API =
  ...
  public @NotNull PlayerTextures resolve(@NotNull Profile profile) throws IOException {
    ...
    val textureData = decoder.decode(response.getProperties().get(0).getValue());
    @Cleanup val bytesReader = new InputStreamReader(new ByteArrayInputStream(textureData));

    return new CachedPlayerTextures<>(
        gson.fromJson(bytesReader, MojangTextures.class).getTextures(),
